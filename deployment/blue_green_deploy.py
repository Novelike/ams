...
class BlueGreenDeploymentCLI:
	...
	async def run_deployment(self, branch: str, force: bool = False) -> int:
		...
		try:
			...
			# 초기화 완료까지 대기
			await deployer._initialize()

			result = await deployer.deploy(branch=branch, force=force)
...
Examples:
  ...
  python blue_green_deploy.py --branch=develop --force
	...
	try:
		...
		exit_code = asyncio.run(cli.run_deployment(args.branch, args.force))
		sys.exit(exit_code)
